...
class DTIPredictor(nn.Module):
    def __init__(self, node_feat_size, edge_feat_size, hidden_feat_size):
        ...
        # CIGconv for l
        self.convl1 = CIGConv(hidden_feat_size, hidden_feat_size)
        self.convl2 = CIGConv(hidden_feat_size, hidden_feat_size)
        self.convl3 = CIGConv(hidden_feat_size, hidden_feat_size)

        # CIGconv for p
        self.convp1 = CIGConv(hidden_feat_size, hidden_feat_size)
        self.convp2 = CIGConv(hidden_feat_size, hidden_feat_size)
        self.convp3 = CIGConv(hidden_feat_size, hidden_feat_size)

        # NIGconv for (l,p)
        self.convlp1 = NIGConv(hidden_feat_size, hidden_feat_size, feat_drop=0.1)
        self.convlp2 = NIGConv(hidden_feat_size, hidden_feat_size, feat_drop=0.1)
        self.convlp3 = NIGConv(hidden_feat_size, hidden_feat_size, feat_drop=0.1)

        # NIGconv for (p,l)
        self.convpl1 = NIGConv(hidden_feat_size, hidden_feat_size, feat_drop=0.1)
        self.convpl2 = NIGConv(hidden_feat_size, hidden_feat_size, feat_drop=0.1)
        self.convpl3 = NIGConv(hidden_feat_size, hidden_feat_size, feat_drop=0.1)

        # first layer of EHIGN
        self.conv1 = HeteroGraphConv(
                {
                    'intra_l' : self.convl1,
                    'intra_p' : self.convp1,
                    'inter_l2p' : self.convlp1,
                    'inter_p2l': self.convpl1
                }
            )
        # second layer of EHIGN
        self.conv2 = HeteroGraphConv(
                {
                    'intra_l' : self.convl2,
                    'intra_p' : self.convp2,
                    'inter_l2p' : self.convlp2,
                    'inter_p2l': self.convpl2
                }
            )
        # third layer of EHIGN
        self.conv3 = HeteroGraphConv(
                {
                    'intra_l' : self.convl3,
                    'intra_p' : self.convp3,
                    'inter_l2p' : self.convlp3,
                    'inter_p2l': self.convpl3
                }
            )
    ...
    def forward(self, bg):
        ...
        rsts = self.conv1(bg, atom_feats)
        rsts = self.conv2(bg, rsts)
        rsts = self.conv3(bg, rsts)
